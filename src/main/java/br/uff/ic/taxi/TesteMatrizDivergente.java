...
public class TesteMatrizDivergente {

	public static void main(String[] args) throws IOException {
		new TesteMatrizDivergente().start();
	}
	...
	private void start() throws IOException {
		...
		Point[][] v = { 
							...
							{new Point(0, 0.010,0.010), new Point(1, 0.010,0.000), new Point(2, 0.000,0.000)},
							{new Point(3, 0.010,0.000), new Point(4, 0.010,-0.010), new Point(5, 0.000,-0.010)},
	...
	private static double[][] mdfU(Config config, Integer map, Integer size, Point[][] v) {
		...
		for (int j = 0; j < 3; j++) {
			for (int i = 0; i < 3; i++) {
				...
				//Verifica se existe vizinho no lado direito e pega o X do vetor 
				if ((i + 1) < 3) {
					r = v[j][i + 1].getX();
				}
		...
		for (int j = 0; j < 3; j++) {
			for (int i = 0; i < 3; i++) {
				//Cria os pontos r-right,l-left,t-top e b-botom relacionados aos vizinhos				
				Point r = new Point();
				if ((i + 1) < 3) {
					r = v[i + 1][j];	
				}
				...
				Point l = new Point();
				if ((i - 1) > -1) {		//Vizinho esquerdo
					l = v[i - 1][j];
				}
				Point b = new Point();
				if ((j - 1) > -1) {		//Vizinho embaixo
					b = v[i][j - 1];
				}
				//Verifica se existe vizinho, caso exista coloca o valor 1 no índice da matriz 
				if (r.getI() != -1) {
					m[r.getI()][c] = r.getV();
				}
				if (t.getI() != -1) {
